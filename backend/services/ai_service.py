"""
AI 服务类 - 处理 OpenAI 调用和工具集成
"""
import requests
import json
import os
from config import Config
from services.tool_service import ToolService
from services.context_engine import (
    ContextEngine, TokenBudget, TokenCounter, RawMessage
)
from services.context_policies import RecencyPolicy, StructurePolicy, DeduplicationPolicy
from models import RequirementConfig


class AIService:
    """AI 服务类"""

    def __init__(self):
        # 优先读 os.environ（Render/生产环境注入），找不到再读 .env（本地开发）
        # 注意：沙盒环境会将 os.environ 中注入代理地址，所以 .env 优先级更高（本地用）
        # Render 生产环境没有 .env 文件，所以必须依赖 os.environ
        env_vars = {}
        env_path = os.path.join(os.path.dirname(__file__), '..', '.env')
        try:
            with open(env_path, 'r') as f:
                for line in f:
                    line = line.strip()
                    if line and not line.startswith('#') and '=' in line:
                        k, v = line.split('=', 1)
                        env_vars[k.strip()] = v.strip()
        except Exception:
            pass

        # 优先级：.env 文件 > os.environ > 默认值
        # 理由：沙盒环境 os.environ 会被注入代理地址覆盖 OPENAI_BASE_URL，
        #       .env 文件记录用户真实配置；Render 上 .env 不存在，自然降级到 os.environ
        self.api_key = (
            env_vars.get('OPENAI_API_KEY')
            or os.environ.get('OPENAI_API_KEY')
            or ''
        )
        self.base_url = (
            env_vars.get('OPENAI_BASE_URL')
            or os.environ.get('OPENAI_BASE_URL')
            or 'https://llm-api.mcisaas.com/v1'
        )
        self.model = (
            env_vars.get('OPENAI_MODEL')
            or os.environ.get('OPENAI_MODEL')
            or 'glm-4.7'
        )

        self.tool_service = ToolService()

        # 初始化上下文引擎（策略链：时间衰减 → 结构加分 → 去重降权）
        self.context_engine = ContextEngine(
            policies=[
                RecencyPolicy(decay_half_life=10),    # 指数衰减，半衰期 10 条
                StructurePolicy(),                     # 问题/代码块/工具结果加分
                DeduplicationPolicy(),                 # 相似内容降权
            ],
            budget=TokenBudget(
                max_total=8000,           # 模型上下文总长度
                reserve_for_output=1500,  # 预留给模型输出
                reserve_for_system=1000,  # 预留给 system prompt
            ),
            counter=TokenCounter(model=self.model)
        )

        print(f"\n🤖 AI 服务初始化")
        print(f"📡 API 地址: {self.base_url}")
        print(f"🔧 模型: {self.model}")
        print(f"🛠️  已加载工具: {', '.join(self.tool_service.list_tools())}")
        print(f"🧠 上下文引擎: RecencyPolicy + StructurePolicy + DeduplicationPolicy\n")

    def chat(self, owner, messages, new_message):
        """
        处理对话，支持工具调用

        Args:
            owner: Owner 对象
            messages: 历史消息列表 (Message 对象)
            new_message: 新消息内容 (字符串)

        Returns:
            str: AI 回复内容
        """
        # 构建 OpenAI 消息格式（集成上下文引擎）
        openai_messages = self._build_messages(owner, messages, new_message)

        # 获取所有可用工具
        tools = self.tool_service.get_all_tools()

        print(f"\n[AI] 开始处理消息: {new_message[:50]}...")
        print(f"[AI] 上下文消息数: {len(openai_messages)}")
        print(f"[AI] 可用工具数: {len(tools)}")

        # 第一次调用 OpenAI
        response = self._call_openai(openai_messages, tools)

        # 检查是否需要调用工具
        if 'tool_calls' in response and response['tool_calls']:
            print(f"[AI] 检测到工具调用: {len(response['tool_calls'])} 个")

            tool_messages = []

            # 执行所有工具调用
            for tool_call in response['tool_calls']:
                tool_name = tool_call['function']['name']
                tool_args_str = tool_call['function']['arguments']

                try:
                    tool_args = json.loads(tool_args_str)
                except json.JSONDecodeError:
                    tool_args = {}

                print(f"[AI] 执行工具: {tool_name}({tool_args})")

                # 执行工具
                tool_result = self.tool_service.execute_tool(
                    tool_name,
                    tool_args,
                    owner
                )

                print(f"[AI] 工具结果: {tool_result[:100]}...")

                # 添加工具消息
                tool_messages.append({
                    'role': 'tool',
                    'tool_call_id': tool_call['id'],
                    'name': tool_name,
                    'content': tool_result
                })

            # 构建包含工具调用的消息
            assistant_message = {
                'role': 'assistant',
                'content': response.get('content'),
                'tool_calls': response['tool_calls']
            }

            # 第二次调用 OpenAI（附带工具结果）
            final_messages = openai_messages + [assistant_message] + tool_messages

            print(f"[AI] 第二次调用 OpenAI（附带工具结果）")
            final_response = self._call_openai(final_messages, tools=None)

            final_content = final_response.get('content', '抱歉，我遇到了一些问题')
            print(f"[AI] 最终回复: {final_content[:100]}...")

            return final_content

        # 无需工具调用，直接返回
        content = response.get('content', '你好！有什么可以帮你的吗？')
        print(f"[AI] 直接回复（无工具调用）: {content[:100]}...")

        return content

    def _build_messages(self, owner, history, new_message):
        """
        构建 OpenAI 消息格式

        使用 ContextEngine 策略化选择历史消息，替代一刀切的 history[-40:]。
        策略链：
          1. RecencyPolicy  - 时间衰减评分（越新越高）
          2. StructurePolicy - 结构特征加分（问题/代码块/工具结果）
          3. DeduplicationPolicy - 重复内容降权

        Token 预算：max_total=8000，预留输出 1500 + system 1000，
        剩余 5500 token 用于历史消息（贪心选取高分消息，保证不超限）。
        """
        # ── 1. 构建系统提示词 ──────────────────────────────────────────
        system_content = (
            owner.ai_persona
            or f'你是 {owner.name} 的 AI 分身，友好、专业地回答访客的问题。'
        )

        # 注入需求收集策略（如已启用）
        strategy_prompt = self._get_requirement_strategy(owner.id)
        if strategy_prompt:
            system_content += f"\n\n{strategy_prompt}"

        # ── 2. 历史消息转换为 RawMessage ──────────────────────────────
        raw_messages = []
        for msg in history:
            raw_messages.append(RawMessage(
                id=str(msg.id),
                session_id=str(msg.session_id),
                role=msg.role,          # 'visitor' / 'ai' / 'owner'
                content=msg.content or '',
                created_at=msg.created_at,
            ))

        # ── 3. 无历史时快速返回 ───────────────────────────────────────
        if not raw_messages:
            return [
                {'role': 'system', 'content': system_content},
                {'role': 'user', 'content': new_message},
            ]

        # ── 4. 调用上下文引擎 ─────────────────────────────────────────
        result = self.context_engine.build(
            raw_messages=raw_messages,
            system_prompt=system_content,
            new_message=new_message,
            context={
                'owner_id': owner.id,
                'owner_name': owner.name,
            }
        )

        # ── 5. 打印决策日志（便于调试） ───────────────────────────────
        trunc = result['truncation_info']
        metrics = result['metrics']
        print(
            f"[ContextEngine] "
            f"原始 {metrics['total_raw']} 条 → 选取 {metrics['total_selected']} 条 | "
            f"Token 用量 {trunc['budget_used']}/{trunc['budget_available']} | "
            f"丢弃 {trunc['messages_dropped']} 条"
        )
        for step in result['decision_log']:
            print(
                f"  [{step['step']} v{step['version']}] "
                f"{step['elapsed_ms']}ms"
            )

        return result['messages']

    def _get_requirement_strategy(self, owner_id: str) -> str:
        """
        获取需求收集策略提示词

        Args:
            owner_id: 主人ID

        Returns:
            str: 策略提示词，如果未启用则返回空字符串
        """
        try:
            config = RequirementConfig.query.get(owner_id)

            # 如果配置不存在，创建默认配置（禁用状态）
            if not config:
                return ''

            # 如果未启用，返回空字符串
            if not config.enabled:
                return ''

            # 返回策略提示词
            return config.strategy_prompt or ''

        except Exception as e:
            print(f"[AI] 获取需求策略时出错: {str(e)}")
            return ''

    def _call_openai(self, messages, tools=None):
        """
        调用 OpenAI API

        Args:
            messages: 消息列表
            tools: 工具列表（可选）

        Returns:
            dict: OpenAI 响应（统一格式）
        """
        base = self.base_url.rstrip('/')
        if base.endswith('/v1') or '/v1/' in base:
            url = f"{base}/chat/completions"
        else:
            url = f"{base}/v1/chat/completions"

        payload = {
            'model': self.model,
            'messages': messages,
            'max_tokens': 1500,
            'temperature': 0.7
        }

        # 如果有工具，添加到请求中
        if tools:
            payload['tools'] = tools
            payload['tool_choice'] = 'auto'

        try:
            response = requests.post(
                url,
                headers={
                    'Authorization': f'Bearer {self.api_key}',
                    'Content-Type': 'application/json'
                },
                json=payload,
                timeout=30
            )

            if response.status_code == 200:
                data = response.json()
                message = data['choices'][0]['message']

                # 返回统一格式
                result = {'content': message.get('content')}
                if message.get('tool_calls'):
                    result['tool_calls'] = message['tool_calls']

                return result
            else:
                print(f"❌ OpenAI API Error: {response.status_code} {response.text}")
                return {'content': '抱歉，AI 服务暂时不可用'}

        except requests.Timeout:
            print(f"❌ OpenAI API Timeout")
            return {'content': '抱歉，AI 服务响应超时'}

        except Exception as e:
            print(f"❌ OpenAI API Exception: {str(e)}")
            return {'content': '抱歉，AI 服务出现错误'}
